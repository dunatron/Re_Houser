import React from 'react';
import { makeStyles } from '@material-ui/core/styles';
import Accordion from '@material-ui/core/Accordion';
import AccordionSummary from '@material-ui/core/AccordionSummary';
import AccordionDetails from '@material-ui/core/AccordionDetails';
import Typography from '@material-ui/core/Typography';
import ExpandMoreIcon from '@material-ui/icons/ExpandMore';

import Section from '@/Components/Section';
import String from '@/Components/Displays/String';

const useStyles = makeStyles(theme => ({
  root: {
    width: '100%',
  },
  heading: {
    fontSize: theme.typography.pxToRem(15),
    fontWeight: theme.typography.fontWeightRegular,
  },
}));

const RefereeListItem = ({
  referee: { id, name, email, phone, relationship },
}) => {
  const classes = useStyles();
  return (
    <div className={classes.root}>
      <Accordion>
        <AccordionSummary
          expandIcon={<ExpandMoreIcon />}
          aria-controls="panel1a-content"
          id="panel1a-header">
          <Typography className={classes.heading}>
            {name} - {relationship}
          </Typography>
        </AccordionSummary>
        <AccordionDetails>
          <Section>
            <String title="Id" value={id} />
            <String title="Name" value={name} />
            <String title="Email" value={email} />
            <String title="Phone" value={phone} />
            <String title="Relationship" value={relationship} />
          </Section>
        </AccordionDetails>
      </Accordion>
    </div>
  );
  return (
    <div>
      <div>id: {id}</div>
      <div>name: {name}</div>
      <div>email: {email}</div>
      <div>phone: {phone}</div>
      <div>relationship: {relationship}</div>
      <div>EDIT</div>
      <div>DELETE</div>
    </div>
  );
};

export default RefereeListItem;
